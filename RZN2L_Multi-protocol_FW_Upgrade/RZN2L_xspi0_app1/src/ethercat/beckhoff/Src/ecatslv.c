...
/*ECATCHANGE_END(V5.13) ECAT1*/
        ...
        case INIT_2_PREOP :
           ...
           UpdateEEPROMLoadedState();
			...
			/* SII firmware version verification , check SII and firmware Revision No */
			retval = ESC_EepromAccess(SII_EEP_IDENTIFY_OFFSET, 8, &Buffer.word[0], ESC_RD);
			FSP_PARAMETER_NOT_USED(retval);
			if(Buffer.dword[SII_EEP_REVESIONNO/2] != tbl_identify[ID_REVISION])
			{
#if (FW_UP_ENABLE_REVNO_CHECK == 1)
				result = ALSTATUSCODE_FW_SII_NOT_MATCH;
#endif
			}
            break;
...
/*ECATCHANGE_START(V5.13) ESM2*/
    ...
    else if ( alControl != (nAlStatus & STATE_MASK) )
    {
		...
		if(stateTrans == BOOT_2_INIT && BL_CheckRebootFlag() == TRUE)
		{
			BL_Reboot();
		}
